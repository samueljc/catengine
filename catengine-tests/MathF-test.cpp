  ...
  public:
    ...
    TEST_METHOD(trig_test)
    {
      ...
      for (size_t i = 0; i < count; ++i) {
        Assert::AreEqual(ad[i], catengine::rad2deg(ar[i]), d_);
        Assert::AreEqual(ar[i], catengine::deg2rad(ad[i]), d_);
      }

      for (float a : ar) {
        Assert::AreEqual(std::sin(a), catengine::sin(a), d_);
        Assert::AreEqual(std::cos(a), catengine::cos(a), d_);
        Assert::AreEqual(std::tan(a), catengine::tan(a), d_);
      }
    }
